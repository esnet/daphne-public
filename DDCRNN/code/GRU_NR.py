...
def get_model(X_train1, y_train, lag):
...
for site in df.columns[1:]:
    ...
    model, history = get_model(X_train1, y_train, lag)
...
folder = 'Results/Results_GRU_NR'
if not os.path.exists(folder):
    os.makedirs(folder)   
df_mae.to_csv(folder + '/MAE_GRU.csv')    
df_rmse.to_csv(folder + '/RMSE_GRU.csv')   
df_mse.to_csv(folder + '/MSE_GRU.csv')  
df_r2.to_csv(folder + '/R2_GRU.csv') 
df_mape.to_csv(folder + '/MAPE_GRU.csv')    
