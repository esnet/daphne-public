...
def generate_seq2seq_data(sp,  batch_size, seq_len, horizon, num_nodes, category, _scaler=None,
    ...
    #    if _scaler:
    #        #data_scaled = _scaler.transform(sp[i])
    #        data_scaled = _scaler[i].transform(sp[i])
    data_scaled = sp.fillna(0)
    data_ = data_scaled.values
    data_expand = np.expand_dims(data_, axis=-1)
    data_list.append(data_expand)

    if add_time_in_day:
        time_ind = (sp[0].index.values - sp[0].index.values.astype('datetime64[D]')) / np.timedelta64(1, 'D')
        time_in_day = np.tile(time_ind, [1, num_nodes, 1]).transpose((2, 1, 0))
        data_list.append(time_in_day)
    if add_day_in_week:
        day_in_week = np.zeros(shape=(num_samples, num_nodes, 7))
        day_in_week[np.arange(num_samples), :, sp.index.dayofweek] = 1
    ...
    temp_x = temp_x.reshape(temp_x.shape[0] * temp_x.shape[1], seq_len, num_nodes, -1)
