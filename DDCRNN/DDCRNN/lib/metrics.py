...
def masked_cosine_tf(preds, labels, null_val=np.nan):
    axis=0
    if np.isnan(null_val):
        mask = ~tf.is_nan(labels)
    else:
    ...
    normalize_preds = tf.nn.l2_normalize(preds, axis=axis)        
    normalize_labels = tf.nn.l2_normalize(labels, axis=axis)
    loss = tf.multiply(normalize_preds, normalize_labels)
    loss = loss * mask
    loss = tf.where(tf.is_nan(loss), tf.zeros_like(loss), loss)
    #cos_similarity=tf.reduce_sum(loss, axis=axis)

    return tf.losses.cosine_distance(normalize_preds, normalize_labels, dim=0)
    #return 1 - tf.reduce_sum(loss, axis=axis)
...
def masked_cosine_loss(null_val):
    def loss(preds, labels):
        mae = masked_cosine_tf(preds=preds, labels=labels, null_val=null_val)
        return mae
    return loss
