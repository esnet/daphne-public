...
Default_task = {'topo_file': "b4.json"}
Demand_file = "b4_demand.txt"
...
class DeeprouteStatEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        # rewards_for_link = self.cal_rewards_link(actions)

        for index in range(len(rewards) - 1):
            rewards[index] -= self.backend.packet_loss_local[index]
            # rewards[index] += rewards_for_link[index]
