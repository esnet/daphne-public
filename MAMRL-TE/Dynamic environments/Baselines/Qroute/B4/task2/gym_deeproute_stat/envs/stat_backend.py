...
class StatBackEnd(object):
    ...
    def __init__(self, flow_lambda, links, nodes, demands, history, seed):
        ...
        self.nodes_Q_ini = self.gen_nodes_Q()
        self.last_destination = {}
        self.real_time_reward = {}
        self.flags = {}
        ...
        if np.random.uniform(0,1) <= Occur_pro:
            ...
            for _ in range(K):
                ...
                while new_f_destination[0].name == node_name:
                    new_f_destination = np.random.choice(self.nodes, 1, replace=False)
                new_f_bw = np.random.choice(self.demands[node_index][new_f_destination[0].index], 1, replace=False)
                ...
                while new_f_destination[0].name == node_name:
                    new_f_destination = np.random.choice(self.nodes, 1, replace=False)
                self.nodes_queues[node_name].append(PacketTraffic(new_f_bw, new_f_lat, new_f_destination[0]))
    ...
    def set_task(self, links):
        self.links = self.gen_edges(links)
        for link in self.links:
            if link.bw == 0:
                self.links_avail[link.name] = 0
            elif self.links_avail[link.name] == 0:
                self.links_avail[link.name] = link.bw
    ...
    def reset_Q(self):
        ...
        for node1 in self.nodes:
            ...
            for node2 in self.nodes:
                for index in range(len(self.nodes_Q[node1.name][node2.name])):
                    self.nodes_Q[node1.name][node2.name][index] = self.nodes_Q_ini[node1.name][node2.name][index]
        ...
        for node in self.nodes:
            ...
            for index in range(self.history):
                ...
                while new_f_destination[0].name == node.name:
                    new_f_destination = np.random.choice(self.nodes, 1, replace=False)
                new_f_bw = np.random.choice(self.demands[node.index][new_f_destination[0].index], 1, replace=False)
                ...
                current_packet = PacketTraffic(new_f_bw, new_f_lat, new_f_destination[0])
            ...
            while len(self.nodes_actions_history[node.name]) < self.history:
    ...
    def update_Q_ini(self, node_name, packet_destination_name, packet_lat, to_node_name, action):

        t = min(self.nodes_Q_ini[to_node_name][packet_destination_name])
        temp = ETA * (t + packet_lat - self.nodes_Q_ini[node_name][packet_destination_name][action])

        self.nodes_Q_ini[node_name][packet_destination_name][action] += temp
    ...
    def take_actions(self, actions):
        ...
        for index, node in enumerate(self.nodes):
            ...
            if len(self.nodes_queues[node.name]) > 0:
                ...
                self.last_destination[node.name] = current_packet.destination
                if self.links_avail[to_link.name] > current_packet.bw:
                    ...
                    self.active_packets.append(current_packet)
                    self.update_Q(node.name, current_packet.destination.name, current_packet.local_lat, to_node_name, actions[index])
                    self.update_Q_ini(node.name, current_packet.destination.name, current_packet.local_lat, to_node_name, actions[index])
                else:
                    self.packet_loss += 1
                    current_packet.local_lat += 200
                    self.update_Q(node.name, current_packet.destination.name, current_packet.local_lat, to_node_name, actions[index])
                self.real_time_reward[node.name] = current_packet.local_lat
                if current_packet.to_node_name == current_packet.destination.name:
                    self.flags[node.name] = 1
                else:
                    self.flags[node.name] = 0
            while len(self.nodes_actions_history[node.name]) > self.history:
                self.nodes_actions_history[node.name].pop(0)
        
        for packet in self.active_packets:
            # print(packet.to_link.name)
